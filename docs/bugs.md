...
## bug_006: Wallet Balance value inconsistency after rest balance-reset

#### Severity: Low

#### Reproduction Steps

1. Perform API POST request for resetting wallet balance (endpoint: api/reset-balance)
2. Observe if response data are valid

#### Actual result:

- Balance value from the response is: 125.5
- Balance value on UI: 120.0
...
- UI balance value is 125.5

#### Business Impact:

The issue where the UI wallet balance displays a lower value than the backend wallet balance
due to REST balance synchronization inconsistency may negatively impact user experience and betting engagement,
but does not directly affect actual wallet funds or transaction integrity.

## bug_007: Place-Bet Rest response returns USD currency

#### Severity: Critical

#### Reproduction Steps

1. Perform API POST request for place-bet (endpoint: api/place-bet)
2. Check the response data

#### Actual result:

- Currency property value: USD
